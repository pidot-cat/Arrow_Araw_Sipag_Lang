...
class AuthProvider with ChangeNotifier {
  ...
  // Falls back to SharedPreferences when offline (Supabase returns null
  // without network because it cannot restore the session token).
  Future<void> _loadAuthState() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      _username = prefs.getString(AppConstants.keyUsername) ?? '';

      // Primary: check live Supabase session
      final user = SupabaseService.currentUser;

      if (user != null) {
        // Online and session is active
        _isLoggedIn = true;
        ...
        // Supabase session not available — may be offline.
        // Fall back to the last persisted login state.
        final savedLogin = prefs.getBool(AppConstants.keyIsLoggedIn) ?? false;
        _isLoggedIn = savedLogin;
        // _username is already loaded from prefs above
      }
    } catch (e) {
      debugPrint('Error loading auth state: $e');
      // On any error, fall back gracefully to not-logged-in
      _isLoggedIn = false;
    } finally {
      _isReady = true;
      notifyListeners();
